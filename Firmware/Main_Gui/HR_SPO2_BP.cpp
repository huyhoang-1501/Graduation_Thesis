...
static const unsigned long FALLBACK_DELAY_MS =60000; // 60 seconds
...
void hrspo2bp_start_fallback_timer(const char *first_called_phone) {
  ...
  Serial.println("[FALLBACK] Timer started - will call default number in 60 seconds if not cancelled");
}
...
static void bp_task_entry(void *pvParameters) {
  ...
  // ===== QUAN TRỌNG: Tạm dừng MAX301 task để tránh xung đột I2C =====
  // MAX30102 (0x57) và AGR12 (0x50) dùng chung bus I2C (Wire trên pin 21/22).
  // Nếu MAX301 task vẫn chạy trong khi BP task đọc AGR12, cả 2 sẽ tranh giành bus
  // → readPressure() luôn thất bại → vòng while (mmHg < 180) không bao giờ thoát
  // → máy bơm chạy mãi, ko đo được huyết áp.
  if (max301TaskHandle != NULL) {
    vTaskSuspend(max301TaskHandle);
    Serial.println("[BP] MAX301 task suspended for I2C exclusivity");
  }
  ...
  // Resume MAX301 task sau khi đo BP xong
  if (max301TaskHandle != NULL) {
    vTaskResume(max301TaskHandle);
    Serial.println("[BP] MAX301 task resumed");
  }
...
void cancelMeasureBloodPressure() {
  if (bpTaskHandle != NULL) {
    ...
    // Resume MAX301 task nếu nó đang bị suspend để tránh treo vĩnh viễn
    if (max301TaskHandle != NULL) {
      vTaskResume(max301TaskHandle);
      Serial.println("[CANCEL] MAX301 task resumed");
    }
  }
}
...
void measureBloodPressure() {
  ...
  while (pressure_mmHg < TARGET_PRESSURE_MMHG) {
    ...
    Serial.printf("BOM: %.1f mmHg / %.0f\n", pressure_mmHg, TARGET_PRESSURE_MMHG);
    delay(40);
  }
  ...
  if (!cancelled) {
    ...
    while (pressure_mmHg > 45.0f && sampleCount < MAX_SAMPLES) {
      ...
      // In áp suất trong quá trình xả chậm (kèm số mẫu đã thu thập)
      Serial.printf("XA_CHAM: %.1f mmHg  sample=%d/%d\n", pressure_mmHg, sampleCount, MAX_SAMPLES);
      delay(50);
    }
  }
  ...
  while (pressure_mmHg > FINAL_DEFLATION_MMHG && (millis() - deflateStart < 5000)) {
    ...
    readPressure(pressure_kPa, pressure_mmHg, raw);
    Serial.printf("XA_NHANH: %.1f mmHg\n", pressure_mmHg);
    delay(10);
  }
